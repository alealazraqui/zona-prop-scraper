...
class Scraper:
    ...
    def parse_estate(self, estate_post):
        ...
        estate = {}
        estate["url"] = "https://www.zonaprop.com.ar" + url

        # Extract location using the specific class name
        location_div = estate_post.find(
            "div", class_="LocationAddress-sc-ge2uzh-0 iylBOA postingAddress"
        )
        if location_div:
            estate["location"] = location_div.get_text().strip()

        # Extract size from the specified h3 class
        size_h3 = estate_post.find(
            "h3", class_="PostingMainFeaturesBlock-sc-1uhtbxc-0 cHDgeO"
        )
        if size_h3:
            span_tag = size_h3.find("span")
            if span_tag:
                estate["size"] = self.parse_text(span_tag.get_text())

        # Extract description from the specified h3 class
        description_h3 = estate_post.find(
            "h3", class_="PostingDescription-sc-i1odl-11 fECErU"
        )
        if description_h3:
            estate["description"] = self.parse_text(description_h3.get_text())
